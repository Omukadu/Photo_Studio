    ...
    <footer className="bg-foreground text-background">
      <div className="grid grid-cols-4 md:grid-cols-8">
        {instagram.map((src) => (
          <div
            key={src}
            className="aspect-square overflow-hidden grayscale opacity-70 hover:opacity-100 transition"
          >
            <img
              src={src}
              alt=""
      ...
      <div className="mx-auto max-w-7xl px-6 py-12">
          ...
          <div className="flex gap-5 md:justify-self-end">
            {[Facebook, Twitter, Instagram].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="text-background/70 hover:text-[var(--gold)]"
              >
        ...
        <p className="mt-10 text-center text-[0.65rem] tracking-[0.3em] uppercase text-background/40">
          © {new Date().getFullYear()} Omkar Chavan Studio — All photographs are
